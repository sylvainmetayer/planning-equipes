...
| `HORLOGE_SIMULEE_AUTORISEE` | `false` | `true` : *Paramètres*, onglet *Instance*, carte « Date et heure simulées », peut figer la date et l'heure que lisent le mode jour J et l'espace animateur, comme sous `quarkus:dev`. Pour un serveur de démonstration ou de recette ; jamais en production — voir [`api.md`](docs/api.md#figer-la-date-du-jour--développement-démonstration-et-recette-uniquement) |
...
| Affichage mural | Un écran pour la TV de la salle de contrôle, lisible à trois mètres : les stands ouverts du jour regroupés par emplacement, la vacation en cours et la suivante, les places libres en rouge, et en bas les places à pourvoir dans les deux heures, les pauses sans relais et la consigne du jour. Il se met à jour seul chaque minute, garde son dernier état s'il perd le réseau, tourne ses pages quand tout ne tient pas, et s'imprime en grand format pour la journée entière. Il s'ouvre par un lien dédié, créé et révoqué depuis les Paramètres — jamais par une session d'administration laissée sur l'écran —, montre le prénom et l'initiale par défaut, et peut se limiter à quelques emplacements, une TV par zone |
