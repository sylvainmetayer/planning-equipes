...
1. Ouvrir <http://localhost:8080> et se connecter (compte `admin`, mot de passe
   ...
   **État de l'édition** s'affiche : la checklist du cycle, chaque étape avec
   ...
   menu comme **Débogage**.
2. Charger la démonstration : sur **Fichiers** (groupe Préparer), onglet
   *Importer*, carte **Exemples**, choisir « Festival réaliste — canicule »
   puis **Charger cet exemple** — ou, sur une édition vide, le lien
   **Charger un exemple** du bloc *Démarrer* de l'accueil. Les référentiels
   sont ensuite modifiables depuis **Stands** (et son onglet
   **Lieux**), **Animateurs**, **Créneaux** et **Typologies**.
...
4. Consulter le résultat dans **Planning** (une journée choisie sur un
   mini-mois, sous cinq rendus : le tableau des stands, le rail des animateurs,
   la carte, les pauses et repas, les changements), ce qui bloque dans
   **Diagnostic**, et le respect des règles dans **Règles du planning**.
5. Exporter le planning : le PDF global de l'organisateur ou l'archive complète
   des plannings individuels (PDF + ICS) depuis la page **Publication**, ou le
   planning d'un seul animateur depuis sa **fiche** (page Animateurs, un clic
   sur son nom).
...
| `HORLOGE_SIMULEE_AUTORISEE` | `false` | `true` : *Paramètres*, onglet *Instance*, carte « Date et heure simulées », peut figer la date et l'heure que lisent l'écran Aujourd'hui et l'espace animateur, comme sous `quarkus:dev`. Pour un serveur de démonstration ou de recette ; jamais en production — voir [`api.md`](docs/api.md#figer-la-date-du-jour--développement-démonstration-et-recette-uniquement) |
| `GLISSER_DEPOSER_ACTIF` | `false` | `true` : le tableau et le rail de la page Planning proposent en plus de glisser une affectation à la souris vers une autre ligne pour la déplacer ou l'échanger. Coupé par défaut tant que le geste est en test ; il ne règle que ce geste pointeur — « Déplacer vers… » du panneau du siège, le déplacement au clavier, l'API et l'assistant MCP restent toujours ouverts |
...
| --- | --- |
| État de l'édition | La page d'accueil, qui prend la forme du moment. En préparation, les étapes du cycle (référentiels, gel, cohérence du référentiel, collecte des disponibilités, ouvertures, besoin, résolution, problèmes, relecture, publication, accusés de réception, foire) en checklist calculée — chaque ligne dit si l'étape est faite, à vérifier, pour information ou à faire, avec le chiffre qui compte et le lien vers l'écran concerné ; la dernière résolution s'y lit en phrases, jamais en score brut ; le gel du référentiel y est rappelé, et se règle dans les Paramètres ; les accusés de réception disent si les relances automatiques sont actives et n'alertent qu'au-delà du délai de relance ; la ligne « Cohérence du référentiel » rassemble toutes les anomalies déjà détectées ailleurs, les anomalies identiques réunies en une ligne avec leur compte (« 16 jours : rien entre 12:00 et 13:00 »), chaque ligne menant à la fiche qui la corrige. Pendant l'événement, la première ligne est celle du jour — J5, stands ouverts, places vides, absents, échanges à arbitrer — et ouvre l'écran Aujourd'hui, à côté du lien vers l'affichage mural ; la checklist se déplie à la demande. Après l'événement, l'archive vient en tête. Sur une édition vide, un bloc « Démarrer » propose d'importer un fichier, de repartir d'une édition ou de charger un exemple. En tête, toujours, « À traiter aujourd'hui » rassemble ce qui attend une décision ce jour-là — déclarations de disponibilité, échanges à arbitrer (en alerte au-delà de l'ancienneté réglée), journées non relues de la semaine qui vient, silencieux à relancer, données modifiées depuis la résolution, personnes à prévenir, et ce qui n'a pas pu partir, tant que la cause demeure : rappel de la veille à une fiche toujours sans adresse, relance — de la nuit ou à la main — à quelqu'un toujours silencieux, sauvegarde de nuit en échec — chaque ligne ouvrant l'écran avec son filtre |
...
| Pause : un trou, ou un relais | Une pause légale due se prend de deux manières, et l'outil n'en connaît pas de troisième : un trou d'au moins la durée réglée dans la grille, ou un relais — un collègue du même stand tenant une place pendant toute la pause. Sans l'un ni l'autre, le plan est refusé comme il le serait pour un dépassement d'heures. Rien à déclarer : ouvrez la place, le solveur s'en sert. En contrepartie, la pause est du repos, donc **déduite partout** : plafonds quotidien et hebdomadaires, heures et équité du planning par personne, Besoin, KPI — un seul nombre d'heures, le même sur tous les écrans |
| Jours d'affilée | Pas trop de jours travaillés consécutifs — **huit par défaut, et le nombre se règle** sur la ligne de la règle, dans Règles du planning : le solveur pénalise les jours en trop sans bloquer, aucun texte n'imposant un décompte glissant, et une organisation qui veut le plafond bloquant l'allume pour son édition depuis la même page. Le seuil vaut mieux que le dosage : sur une grille qui réclame presque tout le monde tous les jours, un jour de plus ou de moins décide si un planning sans écart existe, là où aucun poids ne le peut |
| Temps de trajet entre deux postes | Deux postes d'une même personne sur deux emplacements éloignés doivent laisser le temps d'aller de l'un à l'autre à pied : le temps de marche se calcule à partir de la distance, d'un facteur de détour et d'une vitesse de marche, **réglables sur la ligne de la règle, dans Règles du planning**, avec une tolérance, et le solveur pénalise chaque minute qui manque au battement. Le Rail et la timeline marquent d'un chevron rouge un enchaînement trop serré (« 20 min à pied, 10 min de battement »), ou un trajet pris sur la pause ; la page Problèmes les liste avec le lien vers le Rail du jour, et la fiche d'un emplacement donne le temps de marche vers chacun des autres |
| Covoiturage et arrivée groupée | Dans l'onglet « Covoiturage » de son espace, pendant la collecte des disponibilités mais à part de sa déclaration, un animateur nomme jusqu'à trois coéquipiers avec qui il vient (« Je viens avec… ») ; hors collecte, l'onglet montre où en est sa demande. L'organisation les retrouve dans l'onglet Covoiturage de la page Disponibilités — si chacun a nommé les autres, combien de jours leurs indisponibilités divergent — et décide chaque demande à part : elle valide l'arrivée groupée, ou l'écarte avec un motif facultatif que l'animateur lit ; chacun est prévenu par e-mail. Validée, elle devient un ajustement manuel que seule l'organisation peut annuler, depuis ce même onglet, en prévenant chaque membre — la page des ajustements manuels ne le modifie ni ne le supprime : le solveur cherche à leur donner les mêmes jours et des arrivées et départs proches (tolérance réglable, 30 min par défaut), sans imposer le même stand. Le Rail marque les membres, la page Problèmes liste les jours désalignés, et l'espace dit à chacun, jour par jour, si son covoiturage tient |
...
| Trois façons de lancer le solveur | « Calculer le planning » repart du plan enregistré s'il existe et cherche à l'améliorer, sans rien figer hormis les verrouillages ; « Corriger après un changement » fige ce qui tient et ne recalcule que les postes rouverts ; « Recommencer de zéro » abandonne l'acquis, et le dit avant. Chaque bouton porte sa phrase sur la page. La page annonce d'où partira le prochain calcul et d'où le dernier est parti ; après un calcul, son résultat se lit en phrases, et voir le planning, relire la première journée pas encore relue et publier sont à un clic |
| Ce que le calcul recevra | Avant de lancer, la page Solveur dit ce que le calcul prendra en compte : les verrouillages, les ajustements manuels, les consignes et leurs dates, les déclarations de disponibilité encore en attente — que le calcul ne verra pas —, les règles désactivées et les modifications faites depuis la dernière résolution. Chaque compteur mène à l'écran qui les liste ; la faisabilité et les alertes viennent avant, la volumétrie du problème est repliée en bas |
| Consignes au solveur | Les ajustements manuels, les verrouillages et les consignes réunis sur une page à trois onglets, sous un même sous-titre : ce que le prochain calcul doit respecter. Après chaque saisie, relancer le calcul est proposé sur place — une correction quand un plan existe, qui ne rouvre que ce que la saisie a invalidé |
| Budget de calcul par édition | Chaque édition règle combien de temps un calcul peut durer et quand il s'arrête faute de progrès — seulement une fois le planning faisable, jamais tant que des places restent à pourvoir — sous un plafond que fixe l'exploitant de l'instance, affiché sur l'écran Règles du planning, onglet Calcul. « Revenir au défaut » rend l'édition aux valeurs de l'instance |
...
| Verrouillage partiel | Geler un animateur, un stand, une journée ou un créneau pour que la prochaine résolution n'y touche plus et optimise le reste. Un verrou est un geste sur un plan déjà calculé : il conserve ce qu'une résolution a produit, sans rien dire de ce qui devrait s'y trouver. Avant de poser un verrou, l'écran compte les affectations qu'il figera et mène à la journée qui les montre |
| Relecture d'une journée | Marquer une journée « relue et acceptée », et voir où l'on en est — « 3 journées sur 12 » sur le Solveur, une coche par journée relue sur le mini-mois de la page Planning. Sur cette page, une barre de quatre pastilles compte ce qu'il reste à regarder — sièges vides, pauses sans relais, sièges verrouillés, changements — et filtre le rendu sur chacune ; « Relu et accepté… », dans son menu, affiche les prérequis de ce jour-là (écarts durs, sièges vides, pauses sans relais, postes irremplaçables) et laisse accepter quand même, en le disant. Accepter ne fige rien : le verrouillage est proposé à côté, jamais imposé. La validation porte toujours sur la journée entière, quel que soit le filtre affiché |
...
| Diagnostic — Problèmes | Premier onglet de la page Diagnostic, « ce qui bloque, ce qui manque, ce qui est fragile » : vue unique des blocages, triés par gravité : causes d'infaisabilité détectées sans résolution, et règles encore en défaut après la dernière analyse. Chaque carte dit **où** — les trois stands ou jours les plus touchés, chacun ouvrant la page Planning sur ce stand et ce créneau, panneau du siège ouvert — et **qui** — les personnes les plus concernées, chacune ouvrant sa fiche. Quand une règle a buté sur des exceptions saisies à la main, elles sont nommées une par une, et les pauses légales que personne ne peut relayer. Sous les cartes, « Où se concentrent les écarts » croise les écarts avec les journées, les stands ou les personnes, filtré sur la règle choisie |
| Que faire ? | Sous chaque problème du Diagnostic, les gestes réels qui le règlent, dans l'ordre que le catalogue des règles leur donne, chacun avec sa raison : « Qui peut tenir ce siège ? » s'ouvre sur place et son « Placer » remplit le siège sans quitter le Diagnostic ; les autres ouvrent l'écran déjà positionné — la grille Compétences sur la typologie du stand, la fiche du stand, la ligne de la règle pour régler son plafond, les Horaires des stands, la charge par personne, les verrous, les ajustements en cause affichés seuls, la donnée manquante d'un plancher. Aucune règle de qualité n'a pour seul geste son poids : « Baisser l'importance » vient toujours en dernier, et disparaît quand l'importance est déjà au plus bas. Une journée déjà commencée ne propose rien qui la modifierait |
| Lecture du score | Le score dit en quelques phrases, pour qui n'a jamais entendu parler de « medium » : les règles impératives respectées ou non, les places restées vides et le jour qui en compte le plus, les deux ou trois règles qui pèsent sur l'organisation avec leur part, ce qui vient d'une donnée absente, les ajustements manuels en cause — et, juste après une résolution, ce qui a changé par rapport au plan d'avant. Chaque règle et chaque jour cité mène à sa ligne ; aucune personne n'est nommée. En tête de la page Solveur et du Diagnostic, et pour chaque plan comparé dans Versions du plan |
| Liens du symptôme vers l'écran | Un siège libre, une pause sans relais, un stand de la carte ouvrent le panneau du siège, sans quitter la page Planning ; un goulot de typologie ou une compétence rare mènent aux animateurs qui la détiennent ; un avertissement à l'enregistrement rouvre la fiche ; une cause d'infaisabilité ouvre le créneau ou le stand en cause |
| Horaires des stands | Une grille stand × créneau, saisie et relue au même endroit comme dans un tableur — collage d'un bloc, recopie d'un jour, reprise de la ligne du dessus, application d'une case à toute sa colonne ; une case vide est fermée, et derrière chaque effectif des barres fines montrent l'ouverture retenue, les horaires du stand et la bande de la consigne du jour ; un clic sur une case explique d'où vient son ouverture en une phrase et mène à la fiche du stand, à la journée type ou à la consigne ; la synthèse et les trois erreurs de saisie d'horaires habituelles sont au-dessus, à vérifier avant de lancer un calcul ; la même grille se saisit par journée type, et une dernière vue compare deux à huit stands à un stand de référence, jour par jour et case par case, puis ouvre la copie des horaires de la référence vers les autres ; un jour férié y est marqué par son nom, sans rien interdire |
| Diagnostic — Besoin en animateurs | Effectif minimum estimé à partir des seuls stands et créneaux : dit si le problème est un manque de monde plutôt qu'un manque de temps de calcul, et sur quelle typologie de jeu le vivier de compétents est trop mince. Chaque chiffre mène à l'écran qui le change : une borne aux horaires des stands de la journée la plus chargée ou à la ligne de sa règle dans Règles du planning, chaque journée à ses horaires des stands, un goulot à la grille des compétences, la vérification de faisabilité au Solveur. Chaque journée porte sa marge avant résolution — disponibles moins sièges, sur sa tranche la plus serrée —, dépliable tranche par tranche. La section « À former » ferme l'onglet |
...
| Siège écrit à la main | Poser directement quelqu'un sur un siège — depuis le panneau du siège, l'écran de réparation ou un assistant — est simulé avant d'écrire et refusé si le geste casse une règle dure, en la nommant. Libérer un siège passe toujours : marquer une absence le jour J ne doit jamais être bloqué par le plan qu'on répare |
...
| Diagnostic — Fragilité du planning | Qui est un point de défaillance unique : pour chaque personne, les créneaux qui passeraient sous l'effectif minimum si elle se désiste — et surtout ceux que personne d'autre ne pourrait reprendre — plus les stands tenus par une seule personne compétente. Par personne, trois gestes : « Verrouiller » garde son planning tel quel au prochain calcul, « Qui peut remplacer » ouvre le panneau du siège qu'elle laisserait le plus difficile à tenir, « Former » ouvre la grille des compétences sur les typologies de ses stands ; son nom mène à sa fiche |
| Diagnostic — À former | Au pied de l'onglet Besoin : qui former, typologie par typologie. Pour chaque typologie que le besoin en animateurs dit en manque ou que la fragilité montre tenue par un seul spécialiste, les chiffres de ces deux lectures, les jours en tension, un lien vers la grille des compétences filtrée sur elle, et les candidats — débutants et autonomes de la typologie, jamais un référent ni un polyvalent — classés par jours en tension où ils sont disponibles, souhait puis niveau. Une typologie sans candidat relève du recrutement. Aucun calcul lancé, rien d'écrit ; la section s'exporte en CSV |
| Planning — Panneau Siège | Un clic sur n'importe quelle case de la page Planning — un nom ou un siège libre du tableau, une vacation du rail, un stand de la carte, une pause sans relais, une ligne des changements — ouvre à côté le panneau de ce siège : le stand, le créneau, la personne, les verrous, et en phrases les règles en défaut. Sur un siège tenu : remplacer par un remplaçant viable, déplacer vers un autre siège, libérer — en tenant la personne à l'écart de ce créneau au prochain calcul, case cochée par défaut —, verrouiller ou déverrouiller, voir qui pourrait le tenir, ouvrir la fiche de la personne ; remplacer et libérer ne touchent le siège que s'il est encore tenu par la personne affichée. Sur un siège vide : « Qui peut tenir ce siège ? » — l'ancien banc de touche, limité à ce siège, qui dit qui n'est de service nulle part et quelle règle empêcherait chacun de tenir la place — puis « Placer », qui affecte la personne et la garde au prochain calcul — ni pour une personne qu'un verrou tient hors de ce créneau, ni sur un créneau déjà commencé —, et « Poser un ajustement » prérempli. Un placement qui casserait une règle dure est refusé en la nommant, et un placement qui coûte en qualité le dit. Après chaque geste, le panneau propose de prévenir les personnes concernées (le changement n'est pas publié) et, si une place est restée vide, de corriger le reste par une résolution incrémentale |
| Règles du planning | Tout ce qui décide du plan sur une page, en trois onglets. **Légal** : une ligne par règle obligatoire — son nom en clair, l'article qui la fonde, son état, les seuils qui la règlent (durées hebdomadaires, pause, fenêtre repas…) modifiables sur la ligne même, les écarts de la dernière analyse —, le texte complet et l'historique de ses réglages dans un panneau. **Qualité** : le même tableau pour les règles de qualité et les préférences, avec leur **importance en trois positions — faible, normale, forte** — et le poids exact dans le panneau pour qui le veut. **Calcul** : la durée du calcul, l'arrêt quand plus rien ne progresse, l'e-mail de fin, le début de la soirée et la typologie ninja. Rien ne s'écrit avant « Enregistrer », une règle légale ne s'éteint qu'après confirmation, et « Relancer le calcul » est à un clic |
| Contraintes-plancher | Une règle qui pénalise la quasi-totalité de ce qu'elle évalue mesure une donnée absente du référentiel — aucun souhait déclaré, aucun référent — et ses points sont un plancher qu'aucune résolution ne fera bouger : la page Règles du planning le signale, nomme la donnée et mène à sa saisie, et le score hors plancher s'affiche à côté du score brut dans le récapitulatif du Solveur, les Versions du plan. Rien n'est désactivé à la place de l'organisateur |
...
| Historique des pondérations | Chaque changement de poids ou d'activation d'une règle est gardé avec ses valeurs avant et après, sa date et son origine — écran, assistant, scénario, duplication —, sans rien de nominatif, aussi longtemps que l'édition. Le panneau d'une règle, dans Règles du planning, le pose à côté des résolutions qui ont suivi et de leurs écarts à cette règle ; chaque résolution garde le dosage sous lequel elle a été lancée, que l'Autopsie affiche et filtre, et le Comparateur prévient quand deux plans ont été calculés sous des poids différents |
| Notification de fin de résolution | Un e-mail à l'administrateur dès qu'une résolution se termine — édition, score, faisabilité — pour ne pas rester devant l'écran ; s'active par édition sur la page Règles du planning, onglet *Calcul* |
| Versions du plan | Les résolutions terminées et les instantanés de l'édition dans un seul tableau, du plus récent au plus ancien, le résultat de chacun en phrases, sans rien de nominatif ; les mesures des résolutions survivent à la suppression de l'édition décrite. Un instantané met un planning de côté et le remet en place plus tard — une capture est prise automatiquement avant chaque résolution, et en remettre un périmé demande une confirmation qui nomme la cause. Deux versions cochées, ou une version et le plan en place, se comparent côte à côte sur le score, la couverture, l'équité et les écarts aux règles, toutes éditions confondues sur demande. Les cinq dernières lignes s'affichent sur le Solveur |
| Données modifiées depuis la résolution | L'écran Solveur compte les modifications faites depuis le calcul enregistré, et « Corriger après un changement » les montre avant de proposer quoi rouvrir : le compte par famille (« 3 animateurs, 1 stand, 2 créneaux ») et les dernières lignes de l'historique, avec un lien vers celui-ci. Seules les actions qui changent ce qu'un solve reçoit sont comptées — un envoi ou un export ne périme rien |
...
| Aujourd'hui | L'écran du jour même, pensé pour un téléphone : la journée en une ligne (jour, stands ouverts, vacations restantes) et des compteurs qui mènent où l'on s'en occupe ; on cherche quelqu'un par son nom parmi tout l'effectif, on le marque absent pour la suite de la journée, on voit qui peut reprendre ses postes — avec son numéro de téléphone — et on applique. Le créneau en cours se répare aussi : le siège est coupé à l'heure qu'il est, ce qui a été tenu reste au nom de l'absent, le reste du créneau est remplacé. Les places ouvertes depuis ce matin sont distinguées des trous déjà connus à la publication ; « Prévenir les N personnes » envoie leur planning à jour aux seules personnes concernées. Aucun calcul n'est relancé ; « Afficher sur la TV » crée le lien de l'affichage mural |

### Organiser l'année

| Fonctionnalité | En une phrase |
| --- | --- |
| Éditions | Tout le référentiel et les résultats sont cloisonnés par édition (« Année 2025 », « Année 2026 ») ; deux onglets peuvent travailler sur deux éditions à la fois. « Vider cette édition » repart d'une édition vide sans toucher aux autres ni aux réglages de l'instance |
...
| Consigne d'édition | Un arrêté ferme une bande horaire pour tous les stands sur quelques jours : la consigne la pose sur les dates à venir, propose cochés les stands qui perdent des heures, les rouvre sur des fenêtres de compensation — le soir, le matin — et ne détruit rien de la grille : les vacations fermées perdent leurs sièges, les seuls créneaux ajoutés sont ceux que les réouvertures exigent. Un aperçu chiffré précède chaque pose, prolongation ou levée ; la résolution incrémentale et la publication font le reste, et la levée rend leurs après-midis aux titulaires. Le courriel, l'espace et le PDF disent « horaires modifiés » avec le motif ; une journée passée garde pour toujours la consigne qui l'a gouvernée, d'où « fermer des stands demain », proposé sur le Solveur et sur l'écran Aujourd'hui ; un préréglage nommé (« Plan canicule ») rend chaque vague identique au geste près |

### Référentiels

| Fonctionnalité | En une phrase |
| --- | --- |
| Stands | Au moins une typologie (toujours), effectifs minimum et maximum, restriction aux majeurs, indicateurs premium et effort, horaires en règles récurrentes complétées d'exceptions datées — chaque fenêtre d'ouverture peut nommer son propre effectif, pour un stand qui n'a pas le même besoin le matin, l'après-midi et en nocturne. Les horaires d'un stand se copient sur un autre depuis sa fiche, ou sur toute une sélection depuis la modification en masse — règles, exceptions datées et effectifs de fenêtre compris, avec un signal quand une fenêtre dépasse l'effectif maximum du stand cible ; la fiche de chaque stand liste ses anomalies d'ouverture, dont une fenêtre déclarée à une heure qu'aucune vacation ne couvre. Le tableau se trie sur chaque colonne et dit, pour chaque stand, ses jours ouverts, ses postes et, après calcul, sa couverture. « Ajouter » guide la création — identité, typologies, lieu, puis les créneaux de l'édition à décocher et l'effectif de chacun, sans règle à écrire. Chaque stand a sa fiche : sa grille jour × créneau modifiable sur place, comme dans la grille des Horaires des stands, ses anomalies d'ouverture, ses sièges après calcul, la comparaison avec un autre stand, l'historique de ses modifications, et ses règles en forme condensée |
| Animateurs | Identité, compétences par typologie et niveau, souhaits, jours d'indisponibilité, e-mail et téléphone (facultatifs — le téléphone ne sert qu'à appeler un remplaçant le jour J, et ne sort jamais sur l'affichage mural, un planning ou un export) ; le régime légal se déduit de l'âge à la date de chaque créneau. La liste montre l'âge et le régime, les compétences en pastilles, les indisponibilités et, une fois un planning calculé, les postes de chacun ; elle se filtre sur les mineurs, les managers, une compétence ou un souhait, chaque filtre affiché en puce. Dans le formulaire, un clic sur un jour de la frise de l'édition le marque absent, et une plage « absent du … au … » se pose d'un geste ; les appréciations se saisissent dans la grille des compétences, ouverte sur la personne |
| Fiche animateur | Tout ce qu'on sait d'une personne sur une seule page, et ce qu'on fait pour elle : copier le lien de son espace, lui envoyer son planning par e-mail, la relancer, télécharger son PDF ou son ICS, verrouiller ou libérer tout son planning, poser un ajustement déjà à son nom. Un clic sur un jour de la frise le rend indisponible — et libère dans le même geste ses sièges de ce jour dans le planning enregistré, chacun proposé aussitôt à un remplaçant (« Qui peut tenir ce siège ? ») ; un siège verrouillé reste en place, et la fiche le nomme avec le lien vers les verrouillages ; l'absence s'annule sans rendre les sièges. Son planning jour par jour (amplitude, vacations, coupures, pauses, trajets, chaque vacation menant à la page Planning), sa charge et son équité avec le radar et « Comparer avec… », ses compétences et souhaits modifiables sur place, sa fragilité (verrouiller, former) et le suivi. Précédent et suivant parcourent la liste des animateurs telle qu'on l'a filtrée ; tout nom d'animateur de l'application y mène |
| Grille des compétences | Toutes les appréciations d'un coup, animateurs en lignes et typologies en colonnes : une case se change au clic ou aux touches 0 à 3, les flèches se déplacent comme dans un tableur, une ligne reprend celle du dessus et une case s'applique à toute sa colonne, rien n'est écrit avant « Enregistrer » et chaque fiche garde son garde de modification concurrente. Le souhait se pose ou se retire dans la même case, d'un clic sur le cœur ou de la touche S, et s'enregistre avec le reste ; un bloc copié d'un tableur se colle depuis la case courante, après un aperçu. C'est la seule saisie des appréciations en masse — la fiche d'une personne ne modifie que les siennes — et la grille ne s'échange pas par fichier |
| Créneaux | Les vacations que le solveur remplit — une vacation est une tranche de travail réelle, et il n'y a pas d'autre sorte de créneau. D'abord des journées types nommées — les vacations d'une sorte de journée, un R pour un relais repas — posées sur un calendrier de dates, qui sont celles de l'édition, et appliquées par différence après un aperçu chiffré ; une journée type s'applique aussi sans être mémorisée, pour un besoin ponctuel ; les autres façons — un créneau isolé, la reconnaissance des journées types dans une grille existante, la déduction depuis les horaires des stands, ajoutée à la grille ou à sa place — sont rangées dans un seul menu. La page décrit la grille en place et en affiche le contrôle de cohérence en bandeau, relu seul à chaque visite et après chaque écriture ; chaque créneau compte les stands ouverts et les postes qu'il porte, un lien vers les horaires des stands ce jour-là ; les jours fériés y sont marqués à la saisie, et le contrôle rappelle qu'un mineur n'y travaille pas quand l'édition en compte un ce jour-là |
| Typologies | Vocabulaire commun entre compétences et jeux d'un stand, dont la typologie « ninja » des polyvalents, et le plafond de créneaux qu'un animateur peut y tenir. Chaque typologie montre combien de personnes la maîtrisent, combien l'ont souhaitée et combien de stands la proposent, avec un badge quand elle est orpheline (proposée par un stand, maîtrisée par personne hors polyvalents), fragile (une seule personne) ou inutilisée (aucun stand) ; chaque chiffre mène à la liste qu'il compte, et la typologie ninja se coche dans le tableau même ; l'État de l'édition compte les orphelines avant tout calcul. Une fois un planning calculé, trois colonnes disent ce qu'il en a fait : les postes, les heures, et les personnes affectées sans la compétence |
| Lieux | Les emplacements géolocalisés rattachés aux stands, onglet de la page Stands : une carte les montre tous avec leurs stands, et un marqueur déplacé à la souris enregistre la nouvelle position ; le tableau dit quels stands sont rattachés à chaque lieu, et un lieu se crée depuis le formulaire d'un stand. Ils servent à éviter les déplacements lointains d'un créneau à l'autre |
| Tableaux des référentiels | Les réflexes du tableur sur chaque liste : toutes les colonnes se trient, les identifiants dans leur ordre naturel (T2 avant T10) ; le nom d'une ligne l'ouvre et un menu la modifie, la duplique ou la supprime ; « Exporter cette liste » télécharge en CSV ce que le tableau montre ; un bloc copié d'un tableur se colle sur les lignes, après un aperçu des cases modifiées ; un avertissement d'enregistrement reste marqué sur sa ligne. Un référentiel vide dit ce qu'il contient, mène à l'étape précédente quand elle manque, et propose un exemple sur une édition encore vide |
| Gel du référentiel | Une préparation terminée se fige par famille — stands, créneaux, typologies et emplacements, compétences : plus aucun chemin (formulaire, modification en masse, import, assistant) ne modifie ces fiches jusqu'à la levée du gel, les champs figés s'affichent en lecture seule avec un cadenas. Proposé après le premier calcul et la première publication, jamais imposé ; disponibilités, ajustements, verrous et consignes restent libres, et la levée rappelle si le planning est déjà publié |
...
| Fonctionnalité | En une phrase |
| --- | --- |
| Planning — la page | Une seule page « Planning », le planning en haut : le titre sur une ligne, le jour choisi sur un mini-mois dépliable dont chaque case dit ses sièges vides, sa relecture, son verrou et sa consigne (« aujourd'hui » étant la date du serveur), quatre axes — par jour, par stand, par personne, par typologie — sous les mêmes filtres (stand, animateur, emplacement, typologie) qui restent d'un axe à l'autre, à saisir en deux lettres, la relecture en une barre de quatre pastilles — sièges vides, pauses sans relais, sièges verrouillés, changements — qui filtrent le rendu sur ce qu'elles comptent, et la consigne du jour en une ligne. « Imprimer cette journée » met la journée entière sur papier, « Afficher sur la TV » mène au lien de l'affichage mural. Tout est porté par l'adresse |
| Planning — Par jour, Tableau | La journée en tableau sur toute la largeur de l'écran : un stand par ligne avec son emplacement, un créneau par colonne, un nom par étiquette, les sièges vides en rouge, un stand fermé en gris. Chaque nom et chaque siège vide ouvre le panneau du siège pour corriger à la main — et, si l'instance l'active, le glisser-déposer d'un nom vers un autre stand : déplacement sur un siège libre, échange sur une personne, refusé si une règle dure serait cassée |
| Diagnostic — Tension | Jour croisé avec la tranche horaire, sur le planning enregistré : dans chaque case les personnes réellement libres moins les sièges restés vides, croisées avec la fragilité du plan. Une colonne par heure de début de la grille — un soir qui finit à 22 h en semaine et à minuit le samedi reste une seule colonne. La couleur suit le signe de la marge, rouge en dessous de zéro, vert au-dessus ; le cadre dit la gravité — calme, surveillée, élevée ou critique, par des règles nommées : sièges vides que personne ne peut tenir, siège qu'aucun autre ne pourrait reprendre, stand sans spécialiste, sièges fragiles au-delà de la marge. Une case ouvre le panneau du siège de sa tranche sur la page Planning ; la pire tranche de chaque journée donne ses raisons, avec la fiche de la personne irremplaçable et la fragilité du stand ; une tranche déjà commencée est grisée. La marge avant résolution, elle, est une colonne de l'onglet Besoin |
| Planning — Par jour, Pauses et repas | Où tombent les pauses légales, jour par jour et stand par stand : qui sort au plus tard à quelle heure, pour combien de temps, et qui est là pour relayer — et, à côté, la coupure repas due par chaque journée à cheval sur une fenêtre repas, avec ce qui manque quand la grille ne lui laisse pas de place. Dessous, l'intendance du jour : combien de personnes mangent, à quelle heure et à quel emplacement, midi et soir, sans nommer personne ; tout l'événement s'exporte en CSV. Chaque animateur retrouve ses pauses sur son espace, son PDF et son calendrier |
| Planning — Par jour, Rail | La même journée vue par personne : une ligne par animateur, vacations placées dans le temps, pauses légales posées dessus (en rouge sans relais), lignes vides pour qui reste mobilisable ; si l'instance l'active, une vacation se glisse vers une autre personne, qui la prend ou échange la sienne |
| Planning — Par jour, Carte | La même journée sur la carte des emplacements : un curseur temporel, et chaque lieu coloré selon que ses stands y sont ouverts et pourvus, ouverts avec des places vides, ou ouverts sans personne ; chaque lieu porte le nombre de personnes présentes à cette heure-là, sa taille suit l'effectif, et une grille lieu × tranche horaire — ou lieu × jour, au pic de chaque journée — dit combien de monde se trouve où, pour dimensionner l'encadrement et la logistique d'un lieu |
| Planning — Par stand | Une ligne par stand, une colonne par journée, et dans chaque case les personnes qui tiennent ses sièges — ou leurs compteurs (2/2), ou la couleur seule —, la case colorée selon sa couverture : pourvue, partielle, vide ; un stand fermé ce jour-là se distingue d'un stand vide. À droite, les sièges à pourvoir, les pourvus, la part des heures-sièges tenue et les heures ; en pied, les totaux de chaque journée. Une case ouvre le panneau du siège sur ce stand ce jour-là. En second rendu, la répartition des heures : un rectangle par stand, d'autant plus grand que le stand demande d'heures-sièges, coloré selon la part tenue — rayé sous 80 % —, regroupé par emplacement ou par typologie, restreint à une semaine ou à un jour ; un clic agrandit un groupe, un second ouvre la journée du stand |
| Planning — Par personne | Une ligne par animateur, une colonne par journée, et dans chaque case le stand et les heures, colorée selon qu'il travaille, se repose ou était indisponible — une soirée et une affectation posée sur une journée indisponible y sont marquées. À droite, ce que chacun a reçu, chaque valeur triable : heures et écart à la médiane, heures par semaine, soirée (à partir de l'heure réglable des paramètres légaux, la seule soirée de l'application), week-end, jours fériés, dimanche, « Nuit (paie) » après 22 h, postes et postes exigeants, stands, typologies et emplacements distincts, souhaits et appréciations satisfaits, jours travaillés, de repos et plus longue série ; les colonnes où tout le monde est à zéro sont masquées tant qu'on ne les demande pas. Une frise dessine une barre par personne, qui tient un événement d'un mois sur un écran. Deux exports CSV : les heures pour la paie, et l'équité. Une case ouvre le panneau du siège ; le nom mène à la fiche, où se lit le radar d'une personne |
| Planning — Par typologie | Le planning lu par typologie de jeu, en un tableau : postes, heures, plafond, personnes affectées, compétents, compétents jamais affectés, affectés sans la compétence — chaque chiffre mène à l'écran où l'on agit : Par stand restreint à la typologie, la page Animateurs filtrée, la grille Compétences sur sa colonne |
| Planning — Par jour, Comparer deux jours | Deux journées côte à côte, deux samedis par exemple, sur le tableau ou le rail : un bandeau chiffre pour chacune sièges, pourvus, vides, pauses sans relais, animateurs et stands avec l'écart de l'une sur l'autre, puis une ligne par stand ou par personne alignée des deux côtés, les différences signalées et filtrables. En lecture seule, et l'adresse garde la comparaison pour la partager |
| Messages récents | Sous « À traiter aujourd'hui », que la cloche de la barre ouvre : les alertes des envois de nuit, nommément, et l'historique des messages de l'application — résolutions terminées, contraintes en défaut, erreurs de saisie — consultable après coup, à marquer comme lu ou à effacer |
...
| Affichage mural | Un écran pour la TV de la salle de contrôle, lisible à trois mètres : les stands ouverts du jour regroupés par emplacement, la vacation en cours et la suivante, les places libres depuis ce matin en rouge et les trous déjà connus estompés, les stands fermés sur une seule ligne, et en bas ce qui est nouveau, la vacation qui commence dans la demi-heure avec une place vide, la pause sans relais dans l'heure et la consigne du jour — le même calcul que l'écran Aujourd'hui. Il se met à jour seul chaque minute, garde son dernier état s'il perd le réseau, tourne ses pages quand tout ne tient pas, ne dit « peut différer du planning envoyé » que lorsque c'est le cas, avec le nombre de personnes, et s'imprime en tableau stands × vacations pour la journée entière. Il s'ouvre par un lien dédié, créé et révoqué depuis les Paramètres — jamais par une session d'administration laissée sur l'écran —, montre le prénom et l'initiale par défaut — une lettre de plus, puis le nom entier, là où deux personnes se liraient pareil —, et peut se limiter à quelques emplacements, une TV par zone |
...
| Qui a reçu quelle version | Sur l'écran Diffuser, une table permanente de toute l'édition : la version reçue par chacun, l'état de l'envoi — envoyé, échec avec sa cause, sans e-mail, différé —, le rappel de la veille, la relance et l'accusé de réception, avec par ligne « Renvoyer son planning », « Relancer » et « Différer ». Un envoi en échec se voit et empêche l'accueil de dire « à jour » ; filtres par état et par jour (« ce qui change le 06/09 », compté comme dans la Journée) ; les documents à imprimer disent chacun pour qui ils sont |
| Prévenir quelques personnes | Après un échange accepté ou un remplacement le jour J, « Prévenir les N personnes » publie pour ces personnes seulement ; les autres restent à prévenir à la publication suivante |
...
| Contact de l'organisation | Le téléphone et l'adresse e-mail que l'organisation donne à ses animateurs se règlent par édition, dans Paramètres, onglet *Édition* ; l'espace animateur les reçoit avec le reste de sa page, et une édition dupliquée les garde |
...
| Espace sur téléphone | L'espace se lit debout, sur un téléphone, pendant l'événement : trois onglets — la **journée** (une bande de jours à faire défiler du pouce, l'état de l'instant, une carte par créneau avec son lieu, son itinéraire et ses coéquipiers), l'**aperçu** (quatre chiffres et une frise de toutes les journées, colorée par typologie de stand) et les **coéquipiers** (« suis-je avec quelqu'un ? », par nom, accents et casse ignorés ; les journées où presque tout l'effectif est là ne donnent qu'un nombre). Proposer un échange reste à portée de pouce sous les trois. En tête, avant les onglets : le poste en cours ou le prochain, la confirmation tant qu'elle est due, ce qui a changé, et un seul bouton pour emporter son planning. La navigation tient sur une rangée — « Mes disponibilités » et « Covoiturage » n'apparaissent que pendant la collecte, qu'un bandeau annonce ; le téléphone ou l'adresse de l'organisation est au pied de chaque page ; « Planning communiqué le » n'est affiché que si le courriel est vraiment parti, et sinon l'espace le dit ; « Échanger avec » se cherche en tapant un nom |
...
| Je ne pourrai pas venir | Depuis son espace, toute l'édition — foire et collecte fermées comprises —, l'animateur signale qu'il ne pourra pas être là, sur une journée ou sur un poste, avec un motif facultatif choisi dans une courte liste. L'organisation est prévenue aussitôt par e-mail ; rien ne change au planning tant qu'elle n'a pas constaté l'absence et remplacé, ou classé le signalement. L'animateur relit ce qu'il a signalé et où cela en est, et l'annule tant que rien n'est décidé |
...
| Foire au planning | Les animateurs proposent leurs échanges de créneaux en libre-service ; le collègue visé donne son accord, l'organisation arbitre, rien ne s'applique sans validation. L'écran d'arbitrage s'ouvre sur la file, compte les demandes arrivées depuis la dernière visite, et propose après chaque acceptation ses deux suites sur place : prévenir les deux personnes, corriger le reste du planning. Un échange accepté le dit dans l'espace du demandeur en annonçant qu'il n'entrera dans son planning qu'à sa publication. Ouvrable sur une période datée, comme la collecte des disponibilités ; les deux s'ouvrent et se ferment depuis Paramètres, onglet *Édition*, et les écrans Échanges et Disponibilités en gardent une ligne d'état |
...
| Import / export de scénario | Un fichier YAML décrit une configuration complète d'événement ; l'import valide le fichier et explique ce qui cloche. Les cinquante et quelques exemples livrés avec l'application se chargent depuis la carte **Exemples** de l'onglet Importer, chacun sous un nom lisible avec sa taille et ce qu'il montre, rangés « pour découvrir », « pour tester un cas » et « extrêmes ». Un fichier se vérifie aussi sans rien importer |
