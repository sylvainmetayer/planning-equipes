...
| Échec d'envoi | Un courriel qu'un animateur n'a pas pu recevoir — refusé par le serveur d'envoi, serveur injoignable — se voit : la colonne d'accusé de réception dit « échec d'envoi » au lieu de « silencieux », avec la date et la cause au survol, la synthèse les compte à part, un filtre de la page Animateurs les isole et la fiche le signale. Une adresse refusée n'est plus relancée, ni la nuit ni à la main, tant que la fiche n'a pas été corrigée ; un échec passager n'empêche pas la relance suivante. Aucun suivi de lecture : « parti » veut dire accepté par le serveur d'envoi, rien de plus |
