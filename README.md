...
| Échec d'envoi | Un courriel qu'un animateur n'a pas pu recevoir — refusé par le serveur d'envoi, serveur injoignable — se voit : la colonne d'accusé de réception dit « échec d'envoi » au lieu de « silencieux », avec la date et la cause au survol, la synthèse les compte à part, un filtre de la page Animateurs les isole et la fiche le signale. Plus aucun courriel ne part vers une adresse refusée — publication, planning renvoyé, code d'accès, invitation, relances, rappel de la veille, échanges — tant que l'adresse n'a pas changé : modifier autre chose sur la fiche ne suffit pas, et chaque écran dit qui a été sauté. Un échec passager ne bloque rien, et « Renvoyer les envois en échec » renvoie d'un clic, à chacun, le planning, la relance ou l'invitation qui n'était pas partie. Une relance de nuit qui échoue ne compte pas comme une relance : on peut relancer à la main. Aucun suivi de lecture : « parti » veut dire accepté par le serveur d'envoi, rien de plus |
| Rappels automatiques | La veille au soir, chacun reçoit la liste de ses postes du lendemain ; les silencieux sont relancés une fois, sauf si l'organisateur l'a déjà fait à la main ; les demandes d'échange qui dorment remontent à l'organisation. Rien ne part d'une édition qui ne l'a pas explicitement demandé, une seule édition peut les demander à la fois, et personne ne reçoit deux fois le même message |
