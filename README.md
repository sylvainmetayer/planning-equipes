...
| Créneaux | Découpage temporel que le solveur remplit. D'abord des journées types nommées — les vacations d'une sorte de journée, un R pour un relais repas — posées sur un calendrier de dates, qui sont celles de l'édition, et appliquées par différence après un aperçu chiffré ; ou saisi à la main, en série, généré par le découpage, ou déduit des horaires des stands. La page déclare si la grille contient des amplitudes ou des vacations, en lit le contrôle, et range le découpage dès que ce sont des vacations |
