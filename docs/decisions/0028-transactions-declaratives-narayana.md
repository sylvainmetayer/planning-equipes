...
1. **Narayana est déjà sur le classpath.** `quarkus-flyway` tire `quarkus-agroal` (d'où `agroal-narayana`) et, directement, `quarkus-narayana-jta` (d'où `jakarta.transaction-api`) : retirer Agroal ne ferait pas tomber Narayana, retirer Flyway si. Adopter `@Transactional` n'ajoute aucune dépendance ; la source de données est déjà enrôlable.
