 ...
 */
@QuarkusTest
class ImportScenarioEditionCibleTest {
    ...
    private static void importerScenario() {
    ...
    void sansSectionEditionLaReponseResteMuetteSurLEdition() {
        String sansEdition = SCENARIO.replaceFirst("(?s)edition:.*?\\n\\n", "");
        given().contentType("application/x-yaml")
                .body(sansEdition.getBytes(StandardCharsets.UTF_8))
                .when().post("/api/reference-data/import-scenario-fichier")
                .then()
                .statusCode(200)
