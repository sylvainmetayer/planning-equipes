...
class VacationGeneratorServiceTest {
     ...
     * clamp has two edges — families pushed past either edge all snapped back
     * onto that same edge, so several families kept relaying together. That is
    ...
    void chaqueFamilleReleveAUnInstantDistinct() {
        ...
        assertThat(premieresFins)
                .as("une heure de relais distincte par famille")
                .hasSize(4);
    }
    ...
    void leSecondRelaisDuneJourneeLongueEstLuiAussiEtaleParFamille() {
        ...
        for (int famille = 0; famille < 4; famille++) {
            ...
            List<LocalTime> finsTriees = vacations.stream()
                    ...
                    .distinct()
                    .toList();
            assertThat(finsTriees)
                    .as("famille %d doit avoir au moins deux relais", f)
                    .hasSizeGreaterThan(1);
            secondsRelais.add(finsTriees.get(1));
        }
        assertThat(secondsRelais)
                .as(
                        "le second relais doit tomber à un instant distinct par famille, pas s'écraser sur un bord de fenêtre")
                .hasSize(4);
    }
