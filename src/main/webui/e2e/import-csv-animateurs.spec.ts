...
test("le format attendu et l'aide se lisent avant tout choix de fichier", async ({ browser }) => {
  ...
  await page.goto('/fichiers?cible=animateurs');
...
test('un fichier propre : aperçu, validation, rapport, et les fiches en base', async ({
  ...
  const page = await pageAdmin(browser, admin);
  await page.goto('/fichiers?cible=animateurs');
  // L'écran s'appelle « Fichiers » et la carte dit laquelle : le titre de la
  // page est celui de l'écran, pas celui du fichier.
  await expect(page.locator('#contenu')).toContainText('Fichiers');
