...
describe('EditionStore', () => {
  ...
  function respond(editions: Edition[], courant: Edition): void {
    get.mockImplementation((url: string) => Promise.resolve(url.endsWith('/courant') ? courant : editions));
  }

  it('lists every edition and the one this tab resolved to', async () => {
    respond([edition('A', 'Année 2025', true), edition('B', 'Année 2026')], edition('B', 'Année 2026'));
    await store.reload();
