...
describe('standsModifies et saisie', () => {
  ...
  it('revenir à la valeur d’origine efface la modification', () => {
    const reference = cellulesDepuis(rapport());
    const allerRetour = ecrireCellule(ecrireCellule(reference, { standId: 'A', creneauId: 1 }, 9), { standId: 'A', creneauId: 1 }, 2);
...
describe('deplacement', () => {
  ...
  it('descend sur Entrée et flèche bas, et s’arrête à la dernière ligne', () => {
    expect(deplacement('Enter', { standId: 'A', creneauId: 2 }, STANDS, cols)).toEqual({ standId: 'B', creneauId: 2 });
    expect(deplacement('ArrowDown', { standId: 'B', creneauId: 2 }, STANDS, cols)).toEqual({ standId: 'B', creneauId: 2 });
    expect(deplacement('ArrowUp', { standId: 'B', creneauId: 2 }, STANDS, cols)).toEqual({ standId: 'A', creneauId: 2 });
  });

  it('passe d’un jour à l’autre sur les flèches latérales, Home et End', () => {
    expect(deplacement('ArrowRight', { standId: 'A', creneauId: 3 }, STANDS, cols)).toEqual({ standId: 'A', creneauId: 4 });
    expect(deplacement('ArrowLeft', { standId: 'A', creneauId: 1 }, STANDS, cols)).toEqual({ standId: 'A', creneauId: 1 });
    expect(deplacement('Home', { standId: 'A', creneauId: 5 }, STANDS, cols)).toEqual({ standId: 'A', creneauId: 1 });
    expect(deplacement('End', { standId: 'A', creneauId: 1 }, STANDS, cols)).toEqual({ standId: 'A', creneauId: 5 });
  });
...
describe('collerBloc', () => {
  ...
  it('pose un bloc tabulé depuis la case active, ligne par ligne', () => {
    const cellules = collerBloc(cellulesDepuis(rapport()), '5\t6\n7\t\n', { standId: 'A', creneauId: 2 }, STANDS, cols);
  ...
  it('ignore ce qui déborde de la grille et ce qui n’est pas une valeur', () => {
    const cellules = collerBloc(cellulesDepuis(rapport()), 'x\t9\t9\t9\n1\n1\n1', { standId: 'B', creneauId: 4 }, STANDS, cols);
  ...
  it('lit un zéro ou un tiret collé comme une case fermée', () => {
    const cellules = collerBloc(cellulesDepuis(rapport()), '0\t-', { standId: 'A', creneauId: 1 }, STANDS, cols);
...
describe('recopierJour', () => {
  ...
  it('recopie un jour sur les autres, créneau à créneau par ses heures, sans inventer la nocturne', () => {
    const depart: Cellules = ecrireCellule(cellulesDepuis(rapport()), { standId: 'A', creneauId: 4 }, 7);
  ...
  it('prend pour référence le premier jour renseigné, ou le premier jour', () => {
    ...
    expect(jourDeReference(ecrireCellule(vide, { standId: 'B', creneauId: 5 }, 2), 'B', cols)).toBe('2026-07-09');
