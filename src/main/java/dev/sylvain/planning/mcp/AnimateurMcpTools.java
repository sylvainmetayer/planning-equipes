...
public class AnimateurMcpTools {
    ...
    WrittenAnimateurView modifier_animateur(
            ...
            @ToolArg(
                            description =
                                    "Compétences : id de typologie -> DEBUTANT|AUTONOME|REFERENT (remplace la liste existante)",
                            required = false)
                    Map<String, String> competences,
            @ToolArg(description = "Ids de typologies souhaitées (remplace la liste existante)", required = false)
                    List<String> souhaits,
            @ToolArg(description = "Jours indisponibles AAAA-MM-JJ (remplace la liste existante)", required = false)
                    List<String> joursIndisponibles,
            @ToolArg(
                            description =
    ...
    private static Map<String, NiveauCompetence> competences(Map<String, String> competences) {
        ...
        competences.forEach((typologieId, niveau) ->
                parsed.put(typologieId, McpArgs.enumeration(NiveauCompetence.class, niveau, "niveau de compétence")));
        return parsed;
    }
    ...
    private WrittenAnimateurView written(WrittenAnimateur ecrit) {
        return new WrittenAnimateurView(
                toView(ecrit.animateur(), dateReference()), WarningCodes.of(ecrit.avertissements()));
    }
    ...
    static AnimateurView toView(Animateur animateur, LocalDate reference) {
        ...
        return new AnimateurView(
                animateur.getId(),
                statut,
                animateur.isUnder16On(reference),
                animateur.isManager(),
                animateur.getCompetences(),
                animateur.getSouhaits(),
                animateur.getJoursIndisponibles(),
    ...
    public record AnimateurView(
            String id,
            String statut,
            boolean moinsDe16Ans,
            boolean manager,
            Map<String, NiveauCompetence> competences,
            Set<String> souhaits,
            Set<LocalDate> joursIndisponibles,
            Instant modifieLe) {}
}
