...
/**
 * Turns a {@link Notification} into the {@link Courrier} that says it — the
 ...
 */
@ApplicationScoped
public class RedacteurNotifications {
    ...
    public Optional<Courrier> rediger(Notification notification) {
        return switch (notification) {
            case Notification.CibleSollicitee n -> cibleSollicitee(n);
    ...
    private Optional<Courrier> cibleSollicitee(Notification.CibleSollicitee n) {
        if (sansDestinataire(n.emailCible())) {
        ...
        String corps = new StringBuilder()
                ...
                .toString();
        return Optional.of(new Courrier(n.emailCible(), sujet, corps));
    }

    private Optional<Courrier> demandeDeclinee(Notification.DemandeDeclinee n) {
        if (sansDestinataire(n.emailDemandeur())) {
            return Optional.empty();
        }
        ...
        return Optional.of(new Courrier(n.emailDemandeur(),
                "Planning Équipes — votre demande d'échange a été déclinée", corps.toString()));
    }

    private Optional<Courrier> demandesSoumises(Notification.DemandesSoumises n) {
        List<DemandeEchange> demandes = n.demandes();
        Optional<String> admin = adresseAdmin.resolue();
        ...
        liens.ecranEchanges().ifPresent(lien -> corps
                ...
                .append(lien).append('\n'));
        return Optional.of(new Courrier(admin.get(), sujet, corps.toString()));
    }

    private Optional<Courrier> demandeTranchee(Notification.DemandeTranchee n) {
        if (sansDestinataire(n.emailDemandeur())) {
            return Optional.empty();
        }
        ...
        return Optional.of(new Courrier(n.emailDemandeur(), sujet, corps.toString()));
    }

    private Optional<Courrier> resolutionTerminee(Notification.ResolutionTerminee n) {
        Optional<String> admin = adresseAdmin.resolue();
        if (admin.isEmpty()) {
            return Optional.empty();
        }
    ...
    private static boolean sansDestinataire(String email) {
