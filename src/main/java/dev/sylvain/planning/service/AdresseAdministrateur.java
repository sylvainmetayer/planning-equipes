 ...
 */
@ApplicationScoped
public class AdresseAdministrateur {
     ...
     */
    @Inject
    public AdresseAdministrateur(@ConfigProperty(name = "planning.mail.admin") Optional<String> configuree) {
        this.resolue = configuree == null
                ? Optional.empty()
                : configuree.map(String::trim).filter(adresse -> !adresse.isBlank());
    }
