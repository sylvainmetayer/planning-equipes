 ...
 * persisted solve. Two groups never see each other's rows, so a past edition
 ...
 * the days <b>inside</b> one {@code Groupe}. See {@code docs/groupes.md} §3.</p>
 *
 * <p>{@code defaut} is not "the current group" — that one is designated by the
 * client on every request through the {@code X-Groupe-Id} header. It is the
 * fallback for any caller that designates none.</p>
 */
public class Groupe {
    ...
    public Groupe(String id, String nom, boolean defaut, Instant creeLe) {
        this.id = id;
        this.nom = nom;
        this.defaut = defaut;
        this.creeLe = creeLe;
